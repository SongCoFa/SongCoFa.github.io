    ...
    >
      ...
      >
        <div
          ...
          :class="{ 'leave': data.value.leaveFlag === 'true','bisque': data.item.editedField && data.item.editedField.includes(field.key) }"
        >
          <b-form-input class="no-shadow" type="number" v-model.number="data.value.dataValue" @change="storedChange(data)"></b-form-input>
        </div>
...
export default {
  ...
  computed: {
    ...
    tableFields() {
      ...
      return [...this.Table.fields, ...result, ...this.Table.fieldsAtLast];
    },
    tableItems() {
      // 同一個員工的資料為一群組
      ...
      const res = Object.keys(gatherByEmployee).reduce((a, b) => {
        ...
        gatherByEmployee[`${b}`].forEach((row) => {
          const dataValue = row[`${this.Table.editable}`];
          const leaveFlag = row.leave_flag;
          obj[`${row.date}`] = { dataValue, leaveFlag };
        });
    ...
    exportItems() {
      ...
      const keyName = this.Table.editable;
  ...
  methods: {
    ...
    storedChange(data) {
      ...
      const editableName = `${this.Table.editable}`;

      const dataSet = {
        date: data.field.key,
        emp_ID: data.item.emp_ID,
      };

      dataSet[`${editableName}`] = data.value.dataValue;

      const keyname = [`${dataSet.date}`][`${editableName}`];

      console.log(dataSet);

      // * 1 是為了將字串轉數字
      this.$set(data.item, `${keyname}`, dataSet[`${editableName}`]);
      ...
      for (let i = 0; i < this.stored.length; i++) {
        if (this.stored[i].date === dataSet.date && this.stored[i].emp_ID === dataSet.emp_ID) {
          this.stored[i][`${editableName}`] = dataSet[`${editableName}`];
          return;
        }
      }
