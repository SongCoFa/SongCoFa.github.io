    ...
    >
      ...
      <template v-slot:cell(ScheduleIntermediateStations)="data">
        <span v-if="data.item.intermediate_stations && JSON.parse(data.item.intermediate_stations).length !== 0">
          {{ JSON.parse(data.item.intermediate_stations).map(i => i.station_fullname ? i.station_fullname : '-').join(' → ') }}
        </span>
        <span v-else></span>
      </template>
      <template v-slot:cell(IntermediateETA)="data">
        <span v-if="data.item.intermediate_stations && JSON.parse(data.item.intermediate_stations).length !== 0">
          {{ JSON.parse(data.item.intermediate_stations).map(i => i.ETA ? $moment(i.ETA, 'HH:mm:ss').format('HH:mm:ss') : '-').join(' / ') }}
        </span>
        <span v-else></span>
      </template>
      <template v-slot:cell(IntermediateArrival)="data">
        <span v-if="data.item.intermediate_stations && JSON.parse(data.item.intermediate_stations).length !== 0">
          {{ JSON.parse(data.item.intermediate_stations).map(i => i.arrival ? $moment(i.arrival, 'HH:mm:ss').format('HH:mm:ss') : '-').join(' / ') }}
        </span>
        <span v-else></span>
      </template>
...
import Delete from '@/components/table/Modals/Delete.vue';
